...
;Variables a utilizar
 ...
 ISR:
    BTFSC ((INTCON) and 07Fh), 2 ;Si la bandera del timer 0 se activa
    CALL INT_TMR0 ;entonces ejecutar la interrupción del timer 0

    BTFSC ((PIR1) and 07Fh), 1 ;Si la bandera del timer 2 se activa
    CALL CONTADORES_HORA ;entonces ejecutar los contadores de las horas y meses

    BTFSC ((PIR1) and 07Fh), 0 ;Si la bandera del timer 1 se activa
 ...
 CONFIG_TMR2:
    ...
    BSF ((T2CON) and 07Fh), 3 ;POSTCALER = 0001 = 1:2

    BSF ((T2CON) and 07Fh), 2 ;Activar TMR2

    BSF ((T2CON) and 07Fh), 1
    BCF ((T2CON) and 07Fh), 0 ;PRESCALER = 1:16
    BANKSEL TRISB
 ...
 CONFIG_TMR0:
    ...
    BCF ((OPTION_REG) and 07Fh), 0 ;Prescaler = 110 = 1:128
    BANKSEL PORTA
    REINICIAR_TMR0
    RETURN
